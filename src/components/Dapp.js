...
import ZOPNFTFactoryIF from "../contracts/ZOPNFTFactory.json";
...
export class Dapp extends React.Component {
  ...
  async _intializeEthers() {
    ...
    this.zopnftFactory = new ethers.Contract(ZOPNFTFactoryAddr,ZOPNFTFactoryIF.abi,this._provider.getSigner(0));
  ...
  // While this action is specific to this application, it illustrates how to
  // send a transaction.
  async _mint_updatePrice(_number) {
  ...
  async _mint_createZN(_number) {

    try {
      this._dismissTransactionError();

      let tx = await this.zopnftFactory.createZNtoken('azuki', 'eth', 20220227, 0)
      console.log("transaction sent to mint createZNtoken ", 'azuki', 'eth', 20220227, 0)
      this.setState({ txBeingSent: tx.hash });
      let receipt = await tx.wait()
  ...
  //async _mint_buyOP(_number) {
  async _mint(_number) {

    try {
      this._dismissTransactionError();
      //(date, price, account, amount)
      console.log("transaction sent to mint buyOP ", 20220227, 121, this.state.selectedAddress, 10);
      let tx = await this.zopnftFactory.buyOP(20220227, 121, this.state.selectedAddress, 5);
      console.log("transaction sent to mint buyOP ", 20220227, 121, this.state.selectedAddress, 10);
      this.setState({ txBeingSent: tx.hash });
      let receipt = await tx.wait()
